...
File: https://github.com/mattpoggi/mono-uncertainty/blob/master/networks/decoder.py
...
class ImageDecoder_Supervised(nn.Module):
    def __init__(self, num_ch_enc, scales=range(4), num_output_channels=3, use_skips=True):
        super(ImageDecoder_Supervised, self).__init__()
        ...
        for s in self.scales:
            self.convs[("rgbconv", s)] = Conv3x3(self.num_ch_dec[s], self.num_output_channels)
    ...
    def forward(self, input_features):
        ...
        for i in range(4, -1, -1):
            ...
            if i in self.scales:
                self.outputs[("rgb", i)] = self.convs[("rgbconv", i)](x)
