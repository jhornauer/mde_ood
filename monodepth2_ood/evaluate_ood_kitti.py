...
from networks.bayescap import BayesCap
...
def bayescap_uct(opt, enc, dec, bcap, loader, prefix, plotting):
    ...
    for i, data in enumerate(loader):
        ...
        with torch.no_grad():
            ...
            if plotting:
                # remove outliers for improved plotting
...
def evaluate(opt):
    ...
    if opt.ood_dataset == "places365":
        ood_dataset = ood_datasets.Places365Dataset(os.path.join(opt.ood_data, 'train_256_places365standard'),
                                                    height=height, width=width, scenes='outdoor')
    ...
    if opt.post_process:
        ...
        rgb_decoder = networks.ImageDecoder(encoder.num_ch_enc, num_output_channels=3, scales=opt.scales)
